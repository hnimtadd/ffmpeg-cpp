...
/* static AVFrame *alloc_frame(enum AVPixelFormat pix_fmt, int width, int
 * height) { */
/*   AVFrame *frame; */
/*   int ret; */
/**/
...
int main(int argc, char *argv[]) {
  ...
  ret = avformat_open_input(&inFmtCtx, infile, inFmt, NULL);
  if (ret < 0) {
  ...
  outFmt = av_guess_format("rtsp", outfile, NULL);
  ret = avformat_alloc_output_context2(&outFmtCtx, outFmt, "RTSP", outfile);
  if (ret < 0) {
  ...
  while (true) {
    ...
    if (ret < 0) {
      return HandleError(ret, "cannot read frame");
    }
    ...
    /* pkt->pts = ix++; */
    /* pkt->pts = */
    /*     av_rescale_q(pkt->pts, in_stream->time_base,
     * out_stream->time_base);
     */
    /* pkt->dts = */
    /*     av_rescale_q(pkt->dts, in_stream->time_base,
     * out_stream->time_base);
     */
